...
class DepositDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Deposit Details'), backgroundColor: const Color(0xFF2E3440)),
      body: Padding(
        padding: const EdgeInsets.all(24),
        child: FutureBuilder<DocumentSnapshot>(
          future: deposits.doc(depositId).get(),
          builder: (context, snap) {
            if (snap.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator(color: Color(0xFF88C999)));
            if (!snap.hasData || !snap.data!.exists) return const Center(child: Text('Deposit not found'));
            final data = snap.data!.data()! as Map<String, dynamic>;
            final kioskId = data['kioskId'] ?? 'Unknown';
            final weight = (data['weight'] is int) ? (data['weight'] as int).toDouble() : (data['weight'] ?? 0.0);
            ...
            return FutureBuilder<DocumentSnapshot>(
              ...
              builder: (context, kioskSnap) {
                ...
                if (kioskSnap.hasData && kioskSnap.data!.exists) kioskData = kioskSnap.data!.data() as Map<String, dynamic>;
                return Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text('${(weight/1000).toStringAsFixed(3)} kg', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
                        const SizedBox(height: 8),
                        Text('${weight.toStringAsFixed(2)} g', style: const TextStyle(color: Color(0xFF6B7280))),
                        const Divider(),
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            const Text('Kiosk', style: TextStyle(color: Color(0xFF9CA3AF))),
                            const SizedBox(height: 6),
                            Text(kioskData != null ? '${kioskData['name'] ?? kioskId}' : kioskId, style: const TextStyle(fontWeight: FontWeight.w600)),
                          ]),
                          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                            const Text('Points', style: TextStyle(color: Color(0xFF9CA3AF))),
                            const SizedBox(height: 6),
                            Text('+$points', style: const TextStyle(color: Color(0xFF88C999), fontWeight: FontWeight.w700)),
                          ]),
                        ]),
                        const SizedBox(height: 12),
                        Text('Timestamp', style: const TextStyle(color: Color(0xFF9CA3AF))),
                        const SizedBox(height: 6),
                        Text(fmt(ts), style: const TextStyle(color: Color(0xFF6B7280))),
                        const SizedBox(height: 12),
                        Text('Collected by (userId)', style: const TextStyle(color: Color(0xFF9CA3AF))),
                        const SizedBox(height: 6),
                    ...
                    if (kioskData != null && kioskData.containsKey('location'))
                      Text('Location: ${kioskData['location']}', style: const TextStyle(color: Color(0xFF6B7280))),
                    const Spacer(),
                    ...
                    ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF88C999)),

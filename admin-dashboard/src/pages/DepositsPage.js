...
export default function DepositsPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const list = snap.docs.map((doc) => {
        ...
        const weightGrams = Number(data.weight) || 0;
        ...
        return {
          ...
          timestamp: ts,
          weightKg: weightGrams / 1000, // convert once
        };
      });
  ...
  const kioskOptions = Array.from(
    new Map(deposits.map((d) => [d.kioskId || d.kioskName, d.kioskName])).entries()
  );
  ...
  const exportCSV = () => {
    ...
    const header = ['Time', 'Kiosk', 'User', 'Weight (kg)'];
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-x-auto">
        <table className="min-w-full text-sm text-left">
          ...
          <tbody>
            {filtered.map((d) => (
              <tr key={d.id} className="border-b last:border-0">
                ...
                <td className="px-4 py-2 text-text-sub text-xs">
                  {d.userId.length > 14 ? d.userId.slice(0, 14) + '…' : d.userId}
                </td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-6 text-center text-text-sub">
